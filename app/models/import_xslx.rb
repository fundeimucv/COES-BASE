class ImportXslx
	def self.import_student file, escuela_id, plan_id, tipo_ingreso_id, estado_ingreso, usuario_id, ip, enviar_correo= false
		...
		if not errores_generales.any?
			...
			doc.sheets.first.rows.group_by{|row| row['ci']}.values.each do |row|
				...
				begin
					...
					unless usuario = User.where(ci: row['ci']).limit(1).first
						usuario = User.new
						usuario.ci = row['ci']
					end
					...
					if hay_usuario
						...
						estudiante = Student.find_or_initialize_by(user_id: usuario.ci)
