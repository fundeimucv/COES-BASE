...
class ExportPdf
	...
	def self.section_list section
  		asig = section.subject
		# Variable Locales
		pdf = Prawn::Document.new(top_margin: 20)

		#titulo
		banner_width_logo pdf, "Coordinación Académica", section.school

		section_table_desc pdf, section

		pdf.move_down 10

		pdf.text "Profesor: #{section.teacher.description}", size: 10 if section.teacher
		...
		# enrolls = section.academic_records.include(:user)sort_by{|h| h.estudiante.usuario.apellidos}
		enrolls = section.academic_records.joins(student: :user).order('users.name ASC', 'users.last_name ASC')

		data = [["<b>#</b>", "<b>Cédula</b>", "<b>Estado</b>","<b>Nombre</b>"]]

		enrolls.each_with_index do |h,i|
			if h.student and h.student.user
				label = h.enroll_academic_process.nil? ? '' : h.enroll_academic_process.enroll_status
				data << [i+1, 
				h.student.user.ci,
				label,
				h.student.user.reverse_name]
			end
		end
	...
	def self.section_table_desc pdf, section
		pdf.move_down 10
		...
		data << [ "#{asig.code}", "#{asig.name}", "#{section.code}", "#{section.period.name}", "#{asig.unit_credits}"]
	...
	def self.banner_width_logo pdf, titulo, school = nil, size = nil, student = nil
		...
		if school
			pdf.text school.faculty.name, align: :center, size: size
			pdf.move_down 5
			pdf.text "CONTROL DE ESTUDIOS DE #{school.type_entity.upcase}", align: :center, size: size
			pdf.move_down 5
			pdf.text school.name.upcase, align: :center, size: size

		elsif Faculty.any?
			pdf.text Faculty.first.name, align: :center, size: size
			pdf.move_down 5
		end

		if student and student.user and student.user.picture_profile and student.user.picture_profile.attached?
			# pdf.image Rails.application.routes.url_helpers.rails_blob_path(student.user.picture_profile, only_path: true), at: [450, 720], height: 80 
			require 'open-uri'
			pdf.image open(student.user.picture_profile_as_thumb.service_url), at: [450, 720], height: 80 
		end
